"""
Train a DER++ continual-learning baseline with TITAN as the WSI backbone.
...
from mergeslide.derpp import DerppTITAN, TitanGlobalClassifier
...
def _sample_patches(
    ...
    """Sample a fixed patch budget from the WSI bag and move it to device.

    A non-positive ``k`` keeps the full bag. Sampling happens on CPU before the
    transfer so large slides do not briefly occupy full-bag GPU memory.
    """
    if k is not None:
        k = int(k)
...
def main():
    parser = argparse.ArgumentParser(description="DER++ with TITAN backbone and no prompts")
    ...
    derpp_cfg = cfg.get("derpp", {})

    num_epochs = int(_cfg_value(args, derpp_cfg, "num_epochs", 10))
    lr = float(_cfg_value(args, derpp_cfg, "lr", 1e-5))
    weight_decay = float(_cfg_value(args, derpp_cfg, "weight_decay", 1e-4))
    buffer_size = int(_cfg_value(args, derpp_cfg, "buffer_size", 500))
    alpha = float(_cfg_value(args, derpp_cfg, "alpha", 0.2))
    beta = float(_cfg_value(args, derpp_cfg, "beta", 0.2))
    save_dir = str(_cfg_value(args, derpp_cfg, "save_dir", "./checkpoints/derpp_titan"))
    num_folds = int(_cfg_value(args, derpp_cfg, "num_folds", 10))
    num_workers = int(_cfg_value(args, derpp_cfg, "num_workers", 4))
    k = int(_cfg_value(args, derpp_cfg, "k", 400))
    patch_size = int(_cfg_value(args, derpp_cfg, "patch_size", 1024))
    seed = int(_cfg_value(args, derpp_cfg, "seed", 0))
    ...
    if args.num_tasks is not None:
        ...
        cfg_num_tasks = derpp_cfg.get("num_tasks")
    ...
    for fold_id in tqdm(range(num_folds), desc="folds"):
        ...
        for task_id in range(num_tasks):
            ...
            ckpt_path = os.path.join(fold_dir, f"derpp_titan_after_task_{task_id}.pt")
    ...
    if eval_rows:
        ...
        write_eval_summary_csv(
            os.path.join(save_dir, "derpp_titan_eval_summary_per_fold.csv"),
            eval_rows,
        )
