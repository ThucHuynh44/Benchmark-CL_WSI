"""
Train an ER-ACE continual-learning baseline with TITAN as the WSI backbone.
...
def _summarize_fold(rows: List[dict], fold_id: int) -> dict:
    ...
    for seq_task in range(max_task + 1):
        ...
        bacc_all_seqs.append(float(np.mean([float(row["bacc"]) for row in seq_rows])))

    final_rows = sorted(
        [row for row in rows if int(row["after_task"]) == max_task],
        key=lambda row: int(row["eval_task"]),
    )
    return {
        "fold": fold_id,
...
def main():
    parser = argparse.ArgumentParser(description="ER-ACE with TITAN backbone and no prompts")
    ...
    cfg = load_config(default_filename="er_ace.yaml")
    er_ace_cfg = cfg.get("er_ace", {})

    num_epochs = int(_cfg_value(args, er_ace_cfg, "num_epochs", 10))
    lr = float(_cfg_value(args, er_ace_cfg, "lr", 1e-5))
    weight_decay = float(_cfg_value(args, er_ace_cfg, "weight_decay", 1e-4))
    buffer_size = int(_cfg_value(args, er_ace_cfg, "buffer_size", 500))
    save_dir = str(_cfg_value(args, er_ace_cfg, "save_dir", "./checkpoints/er_ace_titan"))
    num_folds = int(_cfg_value(args, er_ace_cfg, "num_folds", 10))
    num_workers = int(_cfg_value(args, er_ace_cfg, "num_workers", 4))
    k = int(_cfg_value(args, er_ace_cfg, "k", 400))
    patch_size = int(_cfg_value(args, er_ace_cfg, "patch_size", 1024))
    seed = int(_cfg_value(args, er_ace_cfg, "seed", 0))

    freeze_backbone = bool(args.freeze_backbone or er_ace_cfg.get("freeze_backbone", False))
    task_free = bool(args.task_free or er_ace_cfg.get("task_free", False))
    use_amp = False
    eval_after_task = bool((not args.no_eval_after_task) and er_ace_cfg.get("eval_after_task", True))
    mask_unseen_eval = bool((not args.no_mask_unseen_eval) and er_ace_cfg.get("mask_unseen_eval", True))
    use_wandb = (args.use_wandb or er_ace_cfg.get("use_wandb", False)) and not args.disable_wandb
    if use_wandb:
        try:
    ...
    if args.num_tasks is not None:
        ...
        cfg_num_tasks = er_ace_cfg.get("num_tasks")
    ...
    for fold_id in tqdm(range(num_folds), desc="folds"):
        ...
        if use_wandb:
            import wandb
            wandb.init(
                project=er_ace_cfg.get("wandb_project", "MergeSlide-ERACE"),
                entity=er_ace_cfg.get("wandb_entity"),
                group="er_ace_train",
                job_type="train",
                name=f"er_ace_fold_{fold_id}",
        ...
        for task_id in range(num_tasks):
            ...
            print(f"Fold {fold_id}, task {task_id}: ER-ACE training took {elapsed:.1f}s")

            ckpt_path = os.path.join(fold_dir, f"er_ace_titan_after_task_{task_id}.pt")
        ...
        final_path = os.path.join(fold_dir, "er_ace_titan_final.pt")
    ...
    if eval_rows:
        eval_csv = os.path.join(save_dir, "er_ace_titan_eval.csv")
        write_eval_csv(eval_csv, eval_rows)
        write_eval_summary_csv(
            os.path.join(save_dir, "er_ace_titan_eval_summary_per_fold.csv"),
            eval_rows,
        )
